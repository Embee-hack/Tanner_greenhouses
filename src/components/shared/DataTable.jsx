...
export default function DataTable({ columns, data, loading, onRowClick }) {
  return (
    <div className="console-glass overflow-x-auto rounded-2xl border bg-card/85">
        ...
        <tbody>
          {loading ? (
            Array.from({ length: 5 }).map((_, i) => (
              <tr key={i} className="border-b border-border/45">
                {columns.map((col) => (
                  <td key={col.key} className="px-4 py-3">
                    <div className="h-4 bg-muted/80 animate-pulse rounded-full w-3/4" />
                  </td>
                ))}
              </tr>
            ))
